...
const toastContent = `import { createContext, useContext, useState, useCallback, type ReactNode } from 'react'
...
const calendarViewsContent = `import React, { memo, useMemo, useCallback } from 'react'
...
console.log('Running Phase 2 and Phase 3 operations...');

// Write Toast.tsx
fs.writeFileSync(path.join(__dirname, '../src/components/Toast.tsx'), toastContent, 'utf8');

// Write CalendarViews.tsx
fs.writeFileSync(path.join(__dirname, '../src/components/CalendarViews.tsx'), calendarViewsContent, 'utf8');

// Move Types & Test files
const moves = [
  ['src/types/index.ts', 'src/lib/types.ts'],
  ['src/test/setup.ts', 'src/lib/setup.ts'],
  ['src/test/test-utils.tsx', 'src/lib/test-utils.tsx']
];
...
// Delete old individual folders/files
const deletions = [
  'src/context/ToastContext.ts',
  'src/context/providers/ToastProvider.tsx',
  'src/context/providers/ToastProvider.test.tsx',
...
deletions.forEach(delPath => {
  const p = path.join(__dirname, '../', delPath);
  if (fs.existsSync(p)) {
    console.log(`Deleting ${delPath}`);
    try {
      execSync(`git rm -rf "${p}"`, { stdio: 'inherit' });
    } catch (e) {
...
const compIndexPath = path.join(__dirname, '../src/components/index.ts');
if (fs.existsSync(compIndexPath)) {
  let content = fs.readFileSync(compIndexPath, 'utf8');
  content = content.replace(/export \{ default as CalendarMonthView \} from '\.\/CalendarMonthView'/g, '');
  content = content.replace(/export \{ default as CalendarWeekView \} from '\.\/CalendarWeekView'/g, '');
  content = content.replace(/export \{ default as CalendarDayView \} from '\.\/CalendarDayView'/g, '');
  content = content.replace(/export \{ default as CalendarUpcomingWidget \} from '\.\/CalendarUpcomingWidget'/g, 'export { CalendarMonthView, CalendarWeekView, CalendarDayView } from \'./CalendarViews\'\nexport { default as CalendarUpcomingWidget } from \'./CalendarUpcomingWidget\'');
  fs.writeFileSync(compIndexPath, content, 'utf8');
}

// GLOBAL IMPORT PATH MIGRATIONS
console.log('Running global import path migrations...');
...
function walkAll(dir) {
  ...
  list.forEach(file => {
    ...
    if (stat && stat.isDirectory()) {
      results = results.concat(walkAll(file));
    } else {
...
const files = walkAll(path.join(__dirname, '../src'));
...
files.forEach(file => {
  ...
  if (content.includes("'@/types'") || content.includes('"@/types"')) {
    content = content.replace(/['"]@\/types['"]/g, "'@/lib/types'");
    changed = true;
  }

  // Replace test-utils imports
  if (content.includes("'@/test/test-utils'") || content.includes('"@/test/test-utils"')) {
    content = content.replace(/['"]@\/test\/test-utils['"]/g, "'@/lib/test-utils'");
    changed = true;
  }

  // Replace context imports
  if (content.includes('@/context/ToastContext') || content.includes('@/context/providers/ToastProvider')) {
    content = content.replace(/@\/context\/ToastContext/g, '@/components/Toast');
    content = content.replace(/@\/context\/providers\/ToastProvider/g, '@/components/Toast');
    changed = true;
  }
...
console.log('Import path migrations complete!');
