...
function CourseModules({
  ...
  return (
    <Stack gap="md">
      <Card elevated accentLeft className="mb-xl">
        <Card.Header>
          <Stack gap="2xs">
            ...
            <Card key={section.id} elevated hasHeader={true} className="course-section mb-md overflow-hidden shadow-[var(--shadow-md)]">
              ...
              >
                <Stack direction="row" align="center" gap="sm" className="flex-1 min-w-0">
                  <div className={`status-dot w-2 h-2 rounded-[var(--radius-pill)] shrink-0 ${progress > 50 ? 'active bg-success shadow-[0_0_6px_rgba(var(--color-success-rgb),0.3)]' : 'pending bg-slate-300 dark:bg-white/20'}`} />
                  <Heading level={4} className="m-0 truncate">{t(`course_${courseId}_${section.id}_title`)}</Heading>
                </Stack>
                {isExpanded ? (
